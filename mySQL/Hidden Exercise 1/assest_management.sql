...
SELECT employees.name,assets.name,IFNULL(TO_DAYS(assigned_assets.till_at) - TO_DAYS(assigned_assets.from_at),'still assigned') AS 'period(in days)' FROM assigned_assets
JOIN employees 
